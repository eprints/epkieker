...
sub _make_magic_inc {
    ...
    push @MAGICINCS, sub {
        ...
        my $text = <$fh>;
        close($fh);
        %Kieker::WrapEPrints::params = %params;

        $text =~ /(.*?)(__DATA__.*|__END__.*|$)/s;
        my($code, $trailer) = ($1, $2);
        $text = $code.qq[
            ;
            Kieker::WrapEPrints::wrapsubs(
                %Kieker::WrapEPrints::params,
                packages => [qw($module)]
            );
            1;
        ]."\n$trailer";
        ...
        if($params{wrap_inherited}) {
            foreach my $package (@{$nonwildcard_packages}) {
                ...
                foreach my $sub (@subs_to_define) {
                    ...
                    eval qq{
                        sub ${package}::$sub {
                            goto &{\$Kieker::WrapEPrints::INHERITED{"${package}::$sub"}};
                        }
                    };
                    die($@) if($@);
                }
...
# sf2 - prevents Sub::WrapPackages to wrap certain constants defined as methods...
# otherwise this writes FAR too many warnings in the error logs

	# EPrints Constants
	if( $sub =~ /^EPrints::Const/ )
	{
		next;
	}

	# HTTP
	next if( $sub =~ /(OK|DECLINED|DONE|NOT_FOUND|HTTP_|FORBIDDEN)/ );

 	# DB/SQL
	next if( $sub =~ /SQL_/ );

	# EPrints triggers
	next if( $sub =~ /EP_TRIGGER_/ );
...
my $newimport = sub {
    $originallibimport->(@_);
    my %magicincs = map { $_, 1 } @Kieker::WrapEPrints::MAGICINCS;
    @INC = (
